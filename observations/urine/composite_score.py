...
# =============================================================================
# COMPOSITE SCORING — URINE OSD-656 ALAMAR PANEL
#
# Methodology identical to cytokine_dataframe.py serum scoring:
#   - Significance: |log2FC| >= 1.0 (2x fold-change)
#   - Weighted fraction of significant markers per timepoint
#   - Timepoint weights: R+1 = 0.5, others = 1.0
#   - Risk tier: weighted fraction binned to 1-5
#
# Key difference from cytokine scoring:
#   - Domain matching uses EXACT substring list, not marker name exact match,
#     because urine marker names vary in suffix format across datasets.
#   - Substrings chosen to be unambiguous (e.g. 'il_6_' not 'il_6' to avoid
#     matching 'il_60', 'il_16' etc.)
# =============================================================================
...
# =============================================================================
# DOMAIN MAP
# Substrings chosen to be unambiguous — avoid partial matches.
# Verify against your actual marker names with: urine_processed['marker'].unique()
# =============================================================================

DOMAIN_MAP = {
    'inflammation': [
        'il_6',    # IL-6 — master acute phase cytokine
        'il_1b',   # IL-1β — canonical inflammasome
        'tnf',     # TNF-α — master pro-inflammatory
        'ccl2',    # MCP-1 — monocyte recruitment
        'ccl25',   # CCL25 — secondary chemokine
        'cxcl2',   # GROβ — neutrophil chemoattractant
        'mmp3',    # MMP-3 — tissue remodeling
        'mmp8',    # MMP-8 — tissue remodeling
        'ptx3',    # Pentraxin-3 — acute phase
    ],
    'oxidative_stress': [
        'ager',    # RAGE — oxidative stress receptor
        'gzmb',    # Granzyme B — cytotoxic/stress marker
        '8ohdg',   # 8-OHdG — DNA oxidation marker
        'hgf',     # HGF — growth/repair factor
    ]
}

# =============================================================================
# MARKER WEIGHTS
# Matched by substring. More specific substrings listed first to avoid
# ambiguous matches.
# =============================================================================
...
def _find_domain(marker_name, domain_mapping):
    """
    Return domain for a marker by substring match.
    Checks more specific substrings before less specific ones to reduce
    false matches (e.g. 'il_1b' matched before 'il_1').
    """
    marker_clean = str(marker_name).lower()
    for domain, keywords in domain_mapping.items():
        # Sort keywords by length descending — match most specific first
...
def _get_marker_weight(marker_name, weight_map):
    """Return importance weight for a marker by substring match."""
    marker_clean = str(marker_name).lower()
    for kw, weight in sorted(weight_map.items(), key=lambda x: len(x[0]), reverse=True):
        if kw.lower() in marker_clean:
            return weight
    return 0.5
...
def calculate_composite_scores(processed_df, domain_mapping=None,
                               weight_map=None, label='Urine Panel'):
    """
    Score biological domains for each astronaut using postflight urine data.

    Parameters
    ----------
    processed_df   : output of process_urine_df() with log2fc column
    domain_mapping : dict of {domain: [keyword substrings]}. Defaults to DOMAIN_MAP.
    weight_map     : dict of {keyword: float}. Defaults to DOMAIN_WEIGHTS.
    label          : dataset label for printouts

    Returns
    -------
    timepoint_df : per astronaut x timepoint x domain summary
    overall_df   : per astronaut x domain overall risk score
    """
    ...
    if df.empty:
        print("Warning: No domain matches found. Check that marker names contain the keywords in DOMAIN_MAP.")
    ...
    # Printout
    for domain in sorted(timepoint_df['domain'].unique()):
        print(f"\n{'='*65}")
        print(f"  {domain.replace('_', ' ').title()}  ({label})")
        print(f"  Threshold: |log2FC| >= {SIGNIFICANCE_THRESHOLD}  |  R+1 weighted 0.5x")
        print(f"{'='*65}")

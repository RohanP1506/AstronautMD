...
# =============================================================================
# URINE VISUALIZATION — OSD-656 ALAMAR PANEL
# =============================================================================
...
SIGNIFICANCE_THRESHOLD = 1.0  # must match composite_score.py


# =============================================================================
# PLOT 1: COMPOSITE DOMAIN SCORE TIMELINE
# =============================================================================

def plot_composite_timeline(timepoint_df, output_path='fig1_composite_timeline.png'):
    """
    Line plot of weighted fraction of significant markers over time.
    One panel per domain. Each line = one astronaut.
    Uses timepoint_df from calculate_composite_scores().
    """
    df = timepoint_df.copy()
    ...
    fig, axes = plt.subplots(1, len(domains), figsize=(6 * len(domains), 4.5), sharey=True)
    if len(domains) == 1:
        axes = [axes]
    ...
    for ax, domain in zip(axes, domains):
        ...
        for j, astro in enumerate(astronauts):
            ...
            if asub.empty:
                continue
            ax.plot(asub['tp_order'], asub['fraction'],
                    marker='o', markersize=5, linewidth=1.8,
                    color=ASTRONAUT_COLORS[j % len(ASTRONAUT_COLORS)],
                    label=astro, alpha=0.85)

        if launch_x is not None:
            ax.axvline(launch_x, color='black', linestyle='--',
                       linewidth=1.2, alpha=0.5, label='Landing')
        ...
        ax.axhspan(0.5, 1.05, color=color, alpha=0.04)

    axes[0].set_ylabel('Weighted fraction of markers significant')
    axes[-1].legend(title='Astronaut', bbox_to_anchor=(1.02, 1), loc='upper left', fontsize=8)

    fig.suptitle('Urine Domain Scores Over Time\nOSD-656 Alamar Panel',
...
# =============================================================================
# PLOT 2: LOG2FC HEATMAP
# Postflight only. Uses same fix as serum heatmap.
# =============================================================================

def plot_log2fc_heatmap(urine_processed, output_path='fig2_log2fc_heatmap.png',
                        min_abs_fc=0.5):
    """
    Heatmap of log2FC per marker per postflight timepoint.
    One subplot per astronaut. Postflight timepoints only.
    """
    df = urine_processed.copy()
    df = df[~df['timepoint'].str.startswith('L-')]  # postflight only

    df['marker_short'] = (
        df['marker']
        .str.replace('_concentration_normalized_protein_quantity', '', regex=False)
    ...
    for ax, astro in zip(axes, astronauts):
        ...
        pivot = sub.pivot_table(
            index='marker_short', columns='timepoint',
...
# =============================================================================
# PLOT 3: PERSISTENCE BAR CHART
# Fixed: no longer references marker_score (column removed in pipeline refactor).
# Now uses log2fc directly with SIGNIFICANCE_THRESHOLD.
# =============================================================================

def plot_persistence_bars(urine_processed, output_path='fig3_persistence_bars.png',
                          top_n=10):
    """
    Diverging horizontal bar chart per astronaut.
    Bar length = mean log2FC across significant postflight timepoints.
    Annotation = number of postflight timepoints where marker was significant.
    """
    df = urine_processed.copy()
    df['marker_short'] = (
        df['marker']
    ...
    postflight = df[df['timepoint'].str.startswith('R+')].copy()
    postflight['significant'] = postflight['log2fc'].abs() >= SIGNIFICANCE_THRESHOLD
    sig = postflight[postflight['significant']]

    if sig.empty:
        print("No significant markers found for persistence bars.")
        return

    persistence = sig.groupby(['astronaut', 'marker_short']).agg(
        times_significant = ('log2fc', 'count'),
        mean_log2fc       = ('log2fc', 'mean'),
        max_abs_log2fc    = ('log2fc', lambda x: x.abs().max())
    ).reset_index()

    astronauts = sorted(persistence['astronaut'].unique())
    fig, axes  = plt.subplots(1, len(astronauts), figsize=(6 * len(astronauts), 5), sharex=False)
    if len(astronauts) == 1:
        axes = [axes]

    for ax, astro in zip(axes, astronauts):
        sub = persistence[persistence['astronaut'] == astro]
        sub = sub.sort_values(['times_significant', 'max_abs_log2fc'], ascending=True).tail(top_n)
        ...
        ax.set_xlabel('Mean log₂FC')
        ax.set_title(astro, fontweight='bold')
        limit = max(sub['mean_log2fc'].abs().max() + 0.5, 2)
        ax.set_xlim(-limit, limit)
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)

    legend_elements = [
        Patch(facecolor='#E05C5C', label='Upregulated'),
        Patch(facecolor='#5B8DD9', label='Downregulated')
    ]
    fig.legend(handles=legend_elements, loc='lower center', ncol=2,
               bbox_to_anchor=(0.5, -0.06), fontsize=10, frameon=False)

    fig.suptitle(
        'Persistent Post-Flight Urine Marker Shifts\n'
        'Mean log₂FC of significant timepoints  (n = timepoints significant)',
        fontsize=13, fontweight='bold'
    )
    fig.tight_layout()
    fig.savefig(output_path, bbox_inches='tight')
...
# =============================================================================
# MAIN — call from urine.py
# =============================================================================

def generate_all_figures(urine_processed, timepoint_df, output_dir='.'):
    import os
    os.makedirs(output_dir, exist_ok=True)

    plot_composite_timeline(timepoint_df,  f'{output_dir}/fig1_composite_timeline.png')
    plot_log2fc_heatmap(urine_processed,   f'{output_dir}/fig2_log2fc_heatmap.png')
    plot_persistence_bars(urine_processed, f'{output_dir}/fig3_persistence_bars.png')
    print(f"\n[✓] All urine figures saved to: {output_dir}/")

...
def sort_timepoints(timepoints):
    """Sort timepoints chronologically: L- preflight first, then R+ postflight."""
    def key(tp):
...
# =============================================================================
# PLOT 1: LOG2FC HEATMAP
# One subplot per astronaut. Rows = markers, Columns = postflight timepoints.
# Diverging colormap: red = upregulated, blue = downregulated.
# =============================================================================
...
def plot_log2fc_heatmap(processed_df, title_suffix='OSD-575 EVE Panel',
    ...
    Only shows markers with |log2FC| >= min_abs_fc in at least one timepoint.
    Preflight timepoints excluded — baseline is the reference, not a comparison point.
    """
...
# =============================================================================
# PLOT 2: RECOVERY SCORE + LOG2FC OVER TIME (stacked, one figure per domain)
# Top panel: composite marker score per timepoint
# Bottom panel: mean log2FC per timepoint
# =============================================================================

def plot_recovery_trajectories(timepoint_df, long_df, domain_markers,
                               label='Domain', output_path='fig_recovery.png'):
    """
    Two stacked panels per domain:
      - Top: composite fraction score over postflight timepoints
      - Bottom: mean log2FC over postflight timepoints
    Both share the x-axis (timepoints).
    """
    tp_df = timepoint_df.copy()
    tp_df['timepoint_num'] = tp_df['timepoint'].str.extract(r'R\+(\d+)').astype(int)
    tp_df = tp_df.sort_values(['astronaut', 'timepoint_num'])
    timepoint_order = (
        tp_df.drop_duplicates('timepoint')
    ...
    fig, (ax_score, ax_log2fc) = plt.subplots(2, 1, figsize=(11, 9), sharex=True)

    for astronaut, data in tp_df.groupby('astronaut'):
        data = data.set_index('timepoint').reindex(timepoint_order)
        ax_score.plot(timepoint_order, data['fraction'],
    ...
    # Score panel
    ax_score.axhline(0, color='black', linestyle='--', linewidth=1.2, alpha=0.6, label='Baseline')
    ax_score.axhspan(0, 1, alpha=0.04, color='red')
    ax_score.set_ylabel('Weighted fraction of markers significant', fontsize=11)
    ax_score.set_title(f'{label} — domain score', fontsize=12)
    ...
    ax_log2fc.axhspan(0,  2, alpha=0.04, color='red',  label='Upregulated')
    ax_log2fc.axhspan(-2, 0, alpha=0.04, color='blue', label='Downregulated')
...
# Recovery trajectory plots — one per domain
for (tp_df, label, domain_key) in [
    (immune_reg_tp,    'Immune Regulation', 'immune_regulation'),
    (inflammation_tp,  'Inflammation',      'inflammation'),
]:
    plot_recovery_trajectories(
        ...
        MARKERS[domain_key],
        label=label,
        output_path=f'fig_recovery_{domain_key}.png'
    )
